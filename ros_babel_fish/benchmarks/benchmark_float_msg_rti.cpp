...
void callback( const topic_tools::ShapeShifter::ConstPtr &msg )
{
  ...
  parser.deserializeIntoFlatContainer( datatype, absl::Span<uint8_t>( buffer ), &flat_container, 1 );
  parser.applyNameTransform( datatype, flat_container, &renamed_value );

  sum += renamed_value[0].second.extract<float>();
...
{
  ros::init( argc, argv, "ros_babel_fish_benchmark_float_rti" );
  ros::NodeHandle nh;

  ros::Subscriber subscriber = nh.subscribe<topic_tools::ShapeShifter>( "/co2", 4000, &callback );
